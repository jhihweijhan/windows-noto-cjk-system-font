...
if (Test-Path "$latestBackupDir\Fonts.reg") {
    ...
    @(
        ...
        'Segoe UI Variable Text Light', 'Segoe UI Variable Display Light', 'Segoe UI Variable Small Light',
        'Segoe UI Variable Text Semiligh', 'Segoe UI Variable Display Semil', 'Segoe UI Variable Small Semilig',
        'Microsoft JhengHei', 'Microsoft JhengHei UI', 'MS Shell Dlg', 'MS Shell Dlg 2', 'SimSun', 'NSimSun', 'PMingLiU', 'MingLiU', 'Noto Sans TC'
    ) | ForEach-Object {
        Remove-ItemProperty -Path $subKey -Name $_ -ErrorAction SilentlyContinue
    }

    # 還原 FontLink SystemLink
    ...
    foreach ($fontName in $targetFontNames) {
        ...
        if ($existing) {
            $cleaned = @($existing | Where-Object { $_ -notmatch 'NotoSansTC' -and $_ -notmatch 'NotoSansSC' })
            Set-ItemProperty -Path $linkKey -Name $fontName -Value $cleaned -Type MultiString -ErrorAction SilentlyContinue
        }
    }
    Remove-ItemProperty -Path $linkKey -Name 'Noto Sans TC' -ErrorAction SilentlyContinue
    Remove-ItemProperty -Path $linkKey -Name 'Noto Sans SC' -ErrorAction SilentlyContinue
