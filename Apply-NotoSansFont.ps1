...
[FontGDI]::AddFontResource("C:\Windows\Fonts\NotoSansSC-VF.ttf") | Out-Null
    ...
    $scBold = Join-Path $PSScriptRoot "NotoSansSC-Bold.ttf"
    if (Test-Path $scBold) {
        Copy-Item -Path $scBold -Destination "C:\Windows\Fonts" -Force -ErrorAction SilentlyContinue
        [FontGDI]::AddFontResource("C:\Windows\Fonts\NotoSansSC-Bold.ttf") | Out-Null
    }
    $tcBold = Join-Path $PSScriptRoot "NotoSansTC-Bold.ttf"
    if (Test-Path $tcBold) {
        Copy-Item -Path $tcBold -Destination "C:\Windows\Fonts" -Force -ErrorAction SilentlyContinue
        [FontGDI]::AddFontResource("C:\Windows\Fonts\NotoSansTC-Bold.ttf") | Out-Null
    }
...
"NotoSansSC-VF.ttf,Noto Sans SC Thin",
...
# Segoe UI (包含 Windows 11 Segoe UI Variable 全系列)、微軟正黑體、微軟雅黑、Tahoma、Microsoft Sans Serif 首選回退加上 Noto Sans TC / SC

$targetFontNames = (Get-ItemProperty -Path $linkKey -ErrorAction SilentlyContinue).PSObject.Properties |

Where-Object { $_.Name -match '^(Segoe UI|Microsoft JhengHei|Microsoft YaHei|Tahoma|Microsoft Sans Serif)' } |

Select-Object -ExpandProperty Name

foreach ($fontName in $targetFontNames) {

$existing = (Get-ItemProperty -Path $linkKey -Name $fontName -ErrorAction SilentlyContinue).$fontName

$newList = [System.Collections.Generic.List[string]]::new()

$newList.Add("NotoSansTC-VF.ttf,Noto Sans TC")

$newList.Add("NotoSansSC-Bold.ttf,Noto Sans SC Bold")
$newList.Add("SegoeIcons.ttf,Segoe Fluent Icons")
$newList.Add("segmdl2.ttf,Segoe MDL2 Assets")

$newList.Add("NotoSansSC-Bold.ttf,Noto Sans SC")
...
if ($item -notmatch 'NotoSansTC' -and $item -notmatch 'NotoSansSC' -and $item -notmatch 'simsun' -and $item -notmatch 'mingliu') {
...
Set-ItemProperty -Path $linkKey -Name $fontName -Value $newList.ToArray() -Type MultiString -ErrorAction SilentlyContinue

}

Write-Host "  -> FontLink 簡繁雙向回退與表情符號連結完成 (已覆蓋 $($targetFontNames.Count) 個系統字型項目，含 Windows 11 Segoe UI Variable 全系列)！" -ForegroundColor Green
...
foreach ($s in $substitutes) {

Set-ItemProperty -Path $subKey -Name $s -Value 'Noto Sans TC' -ErrorAction SilentlyContinue

}

Set-ItemProperty -Path $subKey -Name 'SimSun' -Value 'Noto Sans SC' -ErrorAction SilentlyContinue

Set-ItemProperty -Path $subKey -Name 'NSimSun' -Value 'Noto Sans SC' -ErrorAction SilentlyContinue

# 5. 導向 HKLM Fonts (讓 Windows 傳統 GDI 應用優先使用 FontSubstitutes)

$fontsKey = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Fonts'

# Segoe UI (傳統 GDI 應用清空值以啟用 FontSubstitutes)

Set-ItemProperty -Path $fontsKey -Name 'Segoe UI (TrueType)' -Value '' -ErrorAction SilentlyContinue

Set-ItemProperty -Path $fontsKey -Name 'Segoe UI Bold (TrueType)' -Value '' -ErrorAction SilentlyContinue

Set-ItemProperty -Path $fontsKey -Name 'Segoe UI Semibold (TrueType)' -Value '' -ErrorAction SilentlyContinue

Set-ItemProperty -Path $fontsKey -Name 'Segoe UI Light (TrueType)' -Value '' -ErrorAction SilentlyContinue

Set-ItemProperty -Path $fontsKey -Name 'Segoe UI Semilight (TrueType)' -Value '' -ErrorAction SilentlyContinue

# 保留 Segoe UI Variable, SimSun, MingLiU 核心字型檔註冊，確保 DirectWrite / WinUI 與系統核心字型度量正常；

# 透過前述 FontLink 與 FontSubstitutes，所有中文字元皆優先無縫呈現 Noto Sans TC / SC，徹底消除路徑列擠字重疊與細明體混雜現象。
